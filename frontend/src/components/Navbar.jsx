    ...
    <header className='sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200 font-sans'>
        <div className='flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto'>
            ...
            onClick={()=>navigate('/')}>
                ...
                {/* brand name */}
                <span className='text-2xl font-extrabold bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500 bg-clip-text text-transparent tracking-wide'>Task Manager</span>
            </div>
            {/* right side */}
            <div className='flex items-center gap-4 '>
                ...
                <div ref={menuRef} className='relative'>
                    <button onClick={handleToggleMenu} className='flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50  transition-col duration-300 border border-transparent hover:border-purple-200'>
                        <div className='relative'>
                            {
                                user.avatar? (
                                    <img src={user.avatar} alt='img' className='w-9 h-9 rounded-full shadow-sm' />
                                ) :(
                    ...
                    {menuOpen && (
                        <ul className='absolute top-14 right-0 w-56 bg-white rounded-2xl shadow-xl border border-purple-100 z-50 overflow-hidden animate-fadeIn'>
                            ...
                            <li className='p-2'>
                                <button onClick={handleLogout} className='flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm hover:bg-purple-50 text-red-600'>
