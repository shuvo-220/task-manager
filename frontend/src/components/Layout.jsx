...
const Layout = ({ user, onLogout }) => {
  ...
  const stats = useMemo(()=>{
    ...
    Math.round((completedTask/totalCount)*100) : 0
  ...
  if(loading) return (
    <div className='min-h-screen bg-gray-50 flex items-center justify-center'>
      <div className='animate-spin rounded-full w-12 h-12 border-t-2 border-b-2 border-purple-500' />
    </div>
  )
  ...
  return (
    <div className='min-h-screen bg-gray-50 '>
      ...
      <div className='ml-0 xl:ml-36 lg:ml-64 md:ml-16 pt-16 p-3 sm:p-4 md:p-4 transition-all duration-300'>
        <div className='grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6 '>
          <div className='xl:col-span-2 space-y-3 sm:space-y-4 '>
            <Outlet context={{ tasks, refresTasks: fetchTasks }} />
          </div>
          ...
          <div className='xl:col-span-1 space-y-4 sm:space-y-6 '>
            <div className='bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-purple-100'>
              ...
              <div className='grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6 '>
                ...
                <StatCard title='Completed' value={stats.completedTask} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-green-500 ' />}/>
                <StatCard title='Pending' value={stats.pendingCount} icon={<Circle className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-fuchsia-500 ' />}/>
                <StatCard title='Compelation Rate' value={`${stats.completionPercentage}%`}icon={<Zap className='w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-500 ' />}/>
              </div>
              <hr className='my-4 sm:my-4 border-purple-100' />
              ...
              <div className='space-y-2 sm:space-y-3 '>
                {tasks.slice(0,3).map((task)=>(
                  <div key={task._id || task.id} className='flex items-center justify-between p-2 sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100'>
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full shrink-0 ml-2 ${task.completedTask ? 'bg-green-100 text-green-700':'bg-fuchsia-100 text-fuchsia-700'}`}>
                ...
                {tasks.length === 0 && (
                  <div className='text-center py-4 sm:py-6 px-2'>
                    <div className='w-12 h-12 sm:w-16 sm:h-16 max-auto sm:mb-4 rounded-full bg-purple-100 flex items-center justify-center  '>
